<?php
/**
 * AJAX handlers for the Muslim Prayer Times plugin.
 *
 * @package muslim-prayer-times
 */
...
/**
 * Convert a time string in 12H (g:i A) or 24H (H:i / G:i) format to H:i.
 *
 * MySQL TIME columns only store 24H values, so all time inputs must be
 * normalised before insert to avoid PM times being silently saved as AM.
 *
 * @param string $time_str Raw time string from CSV/POST.
 * @return string Normalised time in H:i format, or empty string when input is empty.
 */
function muslprti_parse_time_to_24h( $time_str ) {
    if ( empty( trim( $time_str ) ) ) {
        return '';
    }
...
/** AJAX handler for prayer times generation. */
function muslprti_handle_generate() {
    ...
    try {
        ...
        // Reformat times if 12-hour format is requested (Builder always outputs H:i).
        $time_fmt = ( isset( $opts['time_format'] ) && '12hour' === $opts['time_format'] ) ? 'g:i A' : 'H:i';
        if ( 'H:i' !== $time_fmt ) {
...
/** AJAX handler for exporting prayer times from the database. */
function muslprti_handle_export_db() {
    ...
    try {
        ...
        $opts       = get_option( 'muslprti_settings', array() );
        $time_fmt   = ( isset( $opts['time_format'] ) && '24hour' === $opts['time_format'] ) ? 'H:i' : 'g:i A';
        ...
        $results = $wpdb->get_results( // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery,WordPress.DB.DirectDatabaseQuery.NoCaching -- Direct query required; result is time-sensitive and not suitable for long-lived caching.
            $wpdb->prepare(
                "SELECT * FROM {$table_name} WHERE day BETWEEN %s AND %s ORDER BY day ASC", // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared -- Table name is derived from $wpdb->prefix, a trusted constant, not user input.
                $start_date,
                $end_date
            ),
            ARRAY_A
        ...
        for ( $i = 0; $i < 365; $i++ ) {
            ...
            if ( isset( $existing_days[ $date_str ] ) ) {
                ...
                $csv_row = array(
                    ...
                    $row['sunrise'] ? muslprti_date( $time_fmt, strtotime( $row['sunrise'] ) ) : '',
                    $row['dhuhr_athan'] ? muslprti_date( $time_fmt, strtotime( $row['dhuhr_athan'] ) ) : '',
                    $row['dhuhr_iqama'] ? muslprti_date( $time_fmt, strtotime( $row['dhuhr_iqama'] ) ) : '',
                    $row['asr_athan'] ? muslprti_date( $time_fmt, strtotime( $row['asr_athan'] ) ) : '',
                    $row['asr_iqama'] ? muslprti_date( $time_fmt, strtotime( $row['asr_iqama'] ) ) : '',
                    $row['maghrib_athan'] ? muslprti_date( $time_fmt, strtotime( $row['maghrib_athan'] ) ) : '',
                    $row['maghrib_iqama'] ? muslprti_date( $time_fmt, strtotime( $row['maghrib_iqama'] ) ) : '',
...
/** AJAX handler for CSV import preview. */
...
/** AJAX handler for the actual CSV import. */
...
/** AJAX handler for Hijri date preview. */
...
/** AJAX handler for importing SalahAPI settings from a URL. */
...
/** AJAX handler for importing SalahAPI settings from an uploaded file. */
function muslprti_handle_import_salahapi_file() {
    ...
    if ( UPLOAD_ERR_OK !== $salahapi_file['error'] ) {
    ...
    $json_data = $wp_filesystem->get_contents( $salahapi_file['tmp_name'] );
