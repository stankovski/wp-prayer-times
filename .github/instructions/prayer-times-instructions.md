...
Validate every change against WordPress lint before considering it done:

1. **PHP CodeSniffer (WPCS).** Run `phpcs` with the WordPress standard against the changed
   files (or the whole plugin):

   ```sh
   phpcs --standard=WordPress wp-content/plugins/muslim-prayer-times/
   ```
...
2. **Auto-fix** mechanical issues (spacing, alignment, etc.) with `phpcbf`, then re-run
   ...
   ```sh
   phpcbf --standard=WordPress wp-content/plugins/muslim-prayer-times/
   ```
...
A change is not complete until `phpcs` passes cleanly (or only with justified, documented
ignores) and Plugin Check reports no new issues.